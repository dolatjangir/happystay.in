...
import { apartments } from "../../../../lib/propertiesData";
...
export default function ApartmentsPage() {
  ...
  const [furnishing, setFurnishing] = useState<(typeof furnishingFilters)[number]>("Any");
  const [sortOpen, setSortOpen] = useState(false);

  const heroImage = apartments[0]?.image;
  const avgRating =
    apartments.reduce((sum, a) => sum + a.rating, 0) / (apartments.length || 1);

  const visibleApartments = useMemo(() => {
    const filtered = apartments.filter(
      (a) => furnishing === "Any" || a.furnishing === furnishing
    );
    ...
    >
      ...
      <section className="relative h-[38vh] sm:h-[42vh] w-full overflow-hidden">
        ...
        <div className="relative z-10 h-full flex flex-col justify-end px-6 lg:px-20 pb-8 max-w-8xl mx-auto">
          ...
          <p className="text-sm sm:text-base text-white/85 mt-2 max-w-xl">
            Serviced apartments and flats for short stays or month-long
            visits — furnished and ready to move in.
          </p>
      ...
      >
        <div className="px-6 lg:px-20 max-w-8xl mx-auto py-3 flex flex-wrap items-center gap-3">
          {/* Furnishing quick filters */}
          <div className="flex items-center gap-2 overflow-x-auto scrollbar-none">
            <Sofa className="w-4 h-4 shrink-0 mr-0.5" style={{ color: "var(--color-text-faint)" }} />
            {furnishingFilters.map((f) => {
          ...
          <div className="flex items-center gap-2 ml-auto">
            <span className="text-xs font-medium hidden sm:inline" style={{ color: "var(--color-text-muted)" }}>
              {visibleApartments.length} results
            </span>
        ...
        {visibleApartments.length === 0 ? (
          ...
          >
            <p className="font-semibold" style={{ color: "var(--color-text-primary)" }}>
              No apartments match that filter
            </p>
            <p className="text-sm mt-1">Try a different furnishing type.</p>
          </div>
        ) : (
