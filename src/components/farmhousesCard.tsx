...
import { MapPin, Star, Heart, Waves, PawPrint, Users } from "lucide-react";
import type { Property } from "@/components/PropertyCarousel";

export default function FarmhouseCard({ farmhouse }: { farmhouse: Property }) {
    ...
    >
      ...
      <div className="relative h-52 w-full overflow-hidden">
        ...
        {farmhouse.hasPrivatePool && (
          ...
          >
            <Waves className="w-3 h-3" style={{ color: "var(--color-brand-600)" }} />
            Private Pool
          </span>
        )}
        ...
        {farmhouse.badge && (
          <span
            ...
            style={{ backgroundColor: farmhouse.badgeColor ?? "var(--color-badge-favorite)" }}
          >
            {farmhouse.badge}
          </span>
        )}
        ...
        >
          {farmhouse.guests && (
            <span className="flex items-center gap-1">
              ...
              {farmhouse.guests} guests
            </span>
          )}
          <span
            className="flex items-center gap-1"
            style={{ color: farmhouse.petsAllowed ? "var(--color-brand-600)" : "var(--color-text-faint)" }}
          >
            <PawPrint className="w-3.5 h-3.5" />
            {farmhouse.petsAllowed ? "Pets allowed" : "No pets"}
          </span>
        </div>

        {farmhouse.amenities && farmhouse.amenities.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-3">
            {farmhouse.amenities.slice(0, 3).map((a) => (
              <span
                key={a}
        ...
        <div className="mt-auto pt-4 flex items-center justify-between">
          <div>
            ...
            <span className="text-xs font-medium ml-1" style={{ color: "var(--color-text-faint)" }}>
              / {farmhouse.priceUnit ?? "night"}
            </span>
            <p className="text-[11px]" style={{ color: "var(--color-text-faint)" }}>
              {farmhouse.reviewsCount} reviews
            </p>
          </div>
